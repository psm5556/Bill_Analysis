...
import { checkServerKeyConfigured, testFredApiKey } from '@/lib/fredApi';
...
export default function Header({ apiKey, onApiKeyChange, onApiKeyValid }: HeaderProps) {
  ...
  const [serverKeyStatus, setServerKeyStatus] = useState<'checking' | 'ok' | 'none'>('checking');

  useEffect(() => {
    checkServerKeyConfigured().then((ok) => {
      setServerKeyStatus(ok ? 'ok' : 'none');
      if (ok) onApiKeyValid(true);
    });
  ...
  return (
    <header className="bg-slate-900 border-b border-slate-700 px-4 py-4">
      <div className="max-w-7xl mx-auto">
        ...
        {serverKeyStatus === 'checking' && (
          <div className="bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 mb-3 text-sm text-slate-400">
            🔄 서버 FRED API 키 확인 중...
          </div>
        )}
        {serverKeyStatus === 'ok' && (
          <div className="bg-green-950/50 border border-green-700 rounded-lg px-4 py-2 mb-3 text-sm text-green-300">
            ✅ 서버에 FRED API 키가 설정되어 있습니다. 모든 실시간 데이터를 자동으로 불러옵니다.
          </div>
        )}

        {/* Manual key input — only shown if server key is absent */}
        {serverKeyStatus === 'none' && (
